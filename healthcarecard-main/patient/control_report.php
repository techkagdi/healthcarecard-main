...
<body>
  ...
  <section class="home-section">
    ...
    <div class="wrap-login100">
                ...
                <div class="login100-form-title" style="font-size:22px;">
						  Show Report or Not
                         </div>
                         <div class="wrap-input100 validate-input" data-validate = "Report_ID is required">
						<input class="input100" type="text" name="report_id" id="report_id" placeholder="Report_ID" autocomplete="off">
					...
					<!--<a href="index.html">-->
						<button class="login100-form-btn" name="show" id="show">Show
                      </button> 
                      
					</div>
                    <div class="container-login100-form-btn">
                    <button class="login100-form-btn" name="restrict" id="restrict">Restrict
                      </button> </div>
					 <!--</a>--> 
                     </div></div></div>	 
                </form>
...
if(isset($_POST['show'])){
  $did = $_POST['report_id'];
  $sql = "SELECT * FROM disease_table1 WHERE disease_id = '{$did}'";
  $result = mysqli_query($con,$sql);
  if(mysqli_num_rows($result) > 0 ){
    $sql = "UPDATE disease_table1 set d_show = '1' WHERE disease_id = '{$did}'";
    $result = mysqli_query($con,$sql);
    echo "<script>
  Swal.fire({
  icon:'success',
  text:'Report is shown'
  })</script>";
...
if(isset($_POST['restrict'])){
  ...
  $result = mysqli_query($con,$sql);
  if(mysqli_num_rows($result) > 0 ){
    $sql = "UPDATE disease_table1 set d_show = '2' WHERE disease_id = '{$did}'";
    $result = mysqli_query($con,$sql);
    echo "<script>
  Swal.fire({
  icon:'success',
  text:'Report is hidden '
  })</script>";
